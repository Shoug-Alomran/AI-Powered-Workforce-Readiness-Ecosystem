...
function trajectoryMonth(anchor: Date, offset: number) {
...
export default async function RoadmapPage() {
  ...
  const trajectoryAnchor = items[0]?.createdAt ?? ctx.user.createdAt;
  const trajectory = [18,26,readiness.score,Math.min(100,readiness.score+14),Math.min(100,readiness.score+28),Math.min(100,readiness.score+44)].map((score,index)=>({score,label:trajectoryMonth(trajectoryAnchor,index-2)}));
  return <main className="page-shell student-career-roadmap">
    ...
    <section className="student-roadmap-analytics"><article><h2>Readiness trajectory</h2><div className="roadmap-chart">{trajectory.map((point,index)=><i key={point.label} style={{height:`${Math.max(12,point.score)}%`}} className={index===2?"current":index>2?"future":""} aria-label={`${point.label}: ${point.score}% readiness${index>2?" projected":""}`}><span>{index===2?`${point.score}%`:""}</span><small>{point.label}{index>2&&<em>Projected</em>}</small></i>)}</div><div className="roadmap-chart-legend"><span><i/>Recorded</span><span><i/>Current</span><span><i/>Projected</span></div></article><article><h2>Milestone analytics</h2><p><span>Remaining milestones</span><b>{items.length-completed} items</b></p><p><span>Current readiness</span><b>{readiness.score}%</b></p><p><span>Potential readiness gain</span><b className="green">+{items.filter(i=>i.status!=="COMPLETED").reduce((n,i)=>n+i.expectedImpact,0)}</b></p><div className="student-ai-callout"><b>✦</b><p>{items.find(i=>i.status!=="COMPLETED")?.title || "Refresh recommendations for your next milestone."}</p></div></article></section>
