...
export default async function Profile({ searchParams }: { searchParams: Promise<{ upload?: string }> }) {
  ...
  return <main className="page-shell student-passport-page">
    ...
    <div className="student-passport-layout"><div className="stack">
      <section className="card passport-section" id="career-profile"><header><div><span className="eyebrow">PROFILE</span><h2>Career profile</h2></div><small>Keep your identity and goals current</small></header><form action={updateStudentProfile} className="form-grid passport-form"><label>Target career<select className="input" name="targetCareer" defaultValue={s.targetCareer}>{s.targetCareer==="undecided"&&<option value="undecided" disabled>Choose in Career Interests</option>}{tracks.map(t=><option value={t.id} key={t.id}>{t.label}</option>)}</select><small className="muted">For guided selection by major, use Career Interests.</small></label><label>University<input className="input" name="university" defaultValue={s.university??""}/></label><label>Degree<input className="input" name="degree" defaultValue={s.degree??""}/></label><label className="wide">Bio<textarea className="input" name="bio" defaultValue={s.bio??""}/></label><button className="button primary">Save profile</button></form></section>
