...
export default function AccountSettingsForm({ email, verifiedDomain }: { email: string; verifiedDomain: string }) {
  ...
  return <form action={action} className="account-settings-form">
    <label>Email address<input className="input" type="email" name="email" defaultValue={email} required/><small>Must end in <strong>@{verifiedDomain}</strong> to remain organization-verified.</small></label>
