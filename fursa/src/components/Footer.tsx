...
export default async function Footer(){
  ...
  return <footer className="site-footer"><div className="site-footer-grid">
    ...
    <section><h2>{user?"Your Workspace":"Platform"}</h2>{accountLinks?accountLinks.map(([label,href])=><Link href={href} key={href}>{label}</Link>):<><Link href="/#how-it-works">How it Works</Link><Link href="/#ai-ethics">Explainable AI</Link><Link href="/#solutions">Skill Mapping</Link><Link href="/workforce-intelligence">Workforce Intelligence</Link><Link href="/impact">National Impact</Link><Link href="/team">Our Team</Link></>}</section>
