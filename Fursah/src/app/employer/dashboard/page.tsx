...
import { computeJobMatch, JOB_MATCH_MODEL_VERSION } from "@/lib/ai";
...
export default async function EmployerDashboard({searchParams}:{searchParams:Promise<{q?:string;all?:string}>}){
  ...
  // This page reads the clock in two places — "new this week" below, and the
  // generatedAt stamp inside getEmployerIntelligence — so its output depends
  // on when it runs. One boundary here marks the whole route as request-time
  // rather than scattering connection() through the helpers, which is what
  // kept the intelligence modules usable inside `use cache`.
  await connection();
