...
/**
 * Wording for an evidence state, from the student's point of view.
 *
 * "SELF_REPORTED" is accurate but reads like a judgement; what the student
 * needs to know is whether the entry is counting toward their score and what
 * would change that.
 */
function evidenceStateLabel(status: string) {
...
export default async function Profile({ searchParams }: { searchParams: Promise<{ upload?: string; setup?: string }> }) {
  ...
  return <main className="page-shell student-passport-page">
    ...
    <div className="student-passport-layout"><div className="stack">
      <section className="card passport-section" id="career-profile"><header><div><span className="eyebrow">PROFILE</span><h2>Career profile</h2></div><small>Keep your identity and goals current</small></header><form action={updateStudentProfile} className="form-grid passport-form"><CareerMajorSelector tracks={tracks.map(({id,label})=>({id,label}))} initialCareer={s.targetCareer}/><label>University<input className="input" name="university" defaultValue={s.university??""}/></label><label>Degree<input className="input" name="degree" defaultValue={s.degree??""}/></label><label>Graduation year<input className="input" name="graduationYear" type="number" inputMode="numeric" min={new Date().getFullYear()-60} max={new Date().getFullYear()+10} placeholder="e.g. 2026" defaultValue={s.graduationYear??""}/><small className="muted">Optional. Used to show you roles that welcome recent graduates, and never to score you.</small></label><label className="wide">Bio<textarea className="input" name="bio" defaultValue={s.bio??""}/></label><button className="button primary">Save profile</button></form></section>
      ...
      <section className="card passport-section" id="experience"><header><div><span className="eyebrow">WORK HISTORY</span><h2>Experience</h2></div><small>{s.experiences.length} entries</small></header><p className="muted">Record everything you have done. Experience counts toward readiness once an administrator has verified its evidence; until then it stays on your passport and is shown as unscored.</p><div className="passport-list">{s.experiences.map(x=><div className="data-row" key={x.id}><div><strong>{x.title}</strong><div className="muted">{x.org} · {x.months} month(s)</div>{x.reviewNote&&<div className="muted">{x.reviewNote}</div>}</div><div className="actions" style={{margin:0}}><span className="pill">{x.type}</span><span className={`pill status-${x.verificationStatus.toLowerCase().replaceAll("_","-")}`}>{evidenceStateLabel(x.verificationStatus)}</span></div></div>)}</div><form action={addExperience} className="form-grid"><label>Type<select className="input" name="type">{["internship","research","volunteer","competition","hackathon","workshop"].map(x=><option key={x}>{x}</option>)}</select></label><label>Title<input className="input" name="title" required/></label><label>Organization<input className="input" name="org"/></label><label>Months<input className="input" name="months" type="number" min="1" defaultValue="1"/></label><DocumentUpload label="Experience documents"/><button className="button secondary">Add experience</button></form></section>
      <section className="card passport-section" id="projects"><header><div><span className="eyebrow">PORTFOLIO</span><h2>Projects</h2></div><small>{s.projects.length} entries</small></header><p className="muted">Add every project you have built. A project counts toward readiness once an administrator has verified its evidence; until then it stays on your passport and is shown as unscored.</p><div className="passport-list">{s.projects.map(x=><div className="data-row" key={x.id}><div><strong>{x.title}</strong><div className="muted">{x.description}</div>{x.reviewNote&&<div className="muted">{x.reviewNote}</div>}</div><span className={`pill status-${x.verificationStatus.toLowerCase().replaceAll("_","-")}`}>{evidenceStateLabel(x.verificationStatus)}</span></div>)}</div><form action={addProject} className="form-grid"><label>Title<input className="input" name="title" required/></label><label>Description<textarea className="input" name="description"/></label><DocumentUpload label="Project documents"/><button className="button secondary">Add project</button></form></section>
    </div></div>
  </main>;
}
