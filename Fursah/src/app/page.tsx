...
const solutions=[["♙","I’m a student",["Prove what you can do","See what is missing","Take the next best action"]],["▣","I’m hiring",["Define evidence-based requirements","Compare qualified applicants","Record a human decision"]],["♧","I represent a university",["See verified cohort gaps","Connect demand to curriculum","Assign measurable interventions"]]] as const;
...
export default async function Home(){const [students,jobs,employers]=await getHomeCounts();const metrics=[[students,"Illustrative student records"],[jobs,"Illustrative opportunities"],[employers,"Illustrative employers"],[allSkillNames().length,"Skills in shared taxonomy"]] as const;return <main className="home-design">
  ...
  <section id="solutions" className="home-section home-solutions"><header><h2>Start with the decision you need to make</h2><p>Every workspace turns verified evidence into a specific next action.</p></header><div>{solutions.map(([icon,title,items],i)=><article key={title}><i className={`s${i}`}>{icon}</i><h3>{title}</h3><ul>{items.map(label=><li key={label}>✓　{label}</li>)}</ul><Link href="/login/demo">Open workspace →</Link></article>)}</div></section>
